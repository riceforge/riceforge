...
impl DeployManager {
    pub fn plan(rice: &Rice) -> Result<DeployPlan> {
        ...
        if !rice_dir.exists() {
            return Err(RiceForgeError::Deploy(format!(
                "rice '{}' has not been cloned — run install first",
                rice.id
            )));
        }
            ...
            .filter_map(|e| e.ok())
        {
            ...
            if EXCLUDED_AT_ROOT.contains(&first_component.as_str()) {
                continue;
            }
    ...
    pub fn apply(plan: &DeployPlan) -> Result<()> {
        #[cfg(not(unix))]
        return Err(RiceForgeError::UnsupportedPlatform);
        ...
        #[cfg(unix)]
        {
            ...
            for (src, dest) in &plan.links {
                ...
                symlink(src, dest)
                    .map_err(|e| RiceForgeError::Deploy(format!(
                        "symlink {} -> {}: {e}",
                        src.display(),
                        dest.display()
                    )))?;
